...
// ACCESSOR
(__out.d_hi ↦ (~d_hi + (__out.d_lo < d_lo))) ⋆ (__out.d_lo ↦ ((k_USED_LO_MASK & ~d_lo) + 1) % k_BITS_PER_UINT64) && (__out.d_lo & k_UNUSED_LO_MASK) == 0
inline
...
__out == &stream && (spacesPerLevel < 0 ==> true) && (spacesPerLevel >= 0 ==> (__out->tellp() > 0))
static
