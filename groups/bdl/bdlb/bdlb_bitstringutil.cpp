...
// ACCESSOR
// requires: true
// ensures: (__out.d_hi == (~old_d_hi + (__out.d_lo < old_d_lo))) ⋆ (__out.d_lo == (k_USED_LO_MASK & ~old_d_lo) + 1 - (k_BITS_PER_UINT64 * (__out.d_lo < old_d_lo)))
inline
...
// requires: true
// ensures: (__out.d_hi == (lhs.d_hi - rhs.d_hi - (__out.d_lo < (lhs.d_lo - rhs.d_lo)))) && (__out.d_lo == ((lhs.d_lo - rhs.d_lo) & ~k_UNUSED_LO_MASK))
inline
...
// requires: numBits + pos1 <= 64 && numBits + pos2 <= 64
...
// requires: true
// ensures: (spacesPerLevel < 0 ==> (__out == stream && SEPFORALL(0, 1, i, &stream + i ↦ 32))) && (spacesPerLevel >= 0 ==> (__out == stream && SEPFORALL(0, 1, i, &stream + i ↦ 10) ⋆ SEPFORALL(1, level * spacesPerLevel + 1, i, &stream + i ↦ 32)))
static
