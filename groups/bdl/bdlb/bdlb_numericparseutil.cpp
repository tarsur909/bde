...
// requires: true
// ensures: (__out == true ==> (stdlibInput.size() >= 2 && (stdlibInput.substr(0, 2) == "0x" || stdlibInput.substr(0, 2) == "0X"))) && (__out == false ==> (stdlibInput.size() < 2 || (stdlibInput.substr(0, 2) != "0x" && stdlibInput.substr(0, 2) != "0X")))
static
...
namespace bdlb {
                        ...
                        // -----------------------
// CLASS METHODS
// requires: 2 <= base && base <= 36 && ((Ct::isDigit(character) ==> character >= '0' && character <= '9') && (Ct::isAlpha(character) ==> character >= 'a' && character <= 'z' || character >= 'A' && character <= 'Z'))
// ensures: (Ct::isDigit(character) ==> __out == character - '0') && (Ct::isAlpha(character) ==> __out == Ct::toLower(character) - ('a' - 10)) && (!(Ct::isDigit(character) || Ct::isAlpha(character)) ==> __out == -1) && (__out >= 0 ==> __out < base)
