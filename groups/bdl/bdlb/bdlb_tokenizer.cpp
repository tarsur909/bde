...
// MANIPULATORS
__out == *this ⋆ (d_prevDelim_p ↦ old_d_postDelim_p) ⋆ (d_token_p ↦ old_d_cursor_p) ⋆ (d_postDelim_p ↦ old_d_cursor_p) ⋆ (d_endFlag == true || d_endFlag == false)
...
// ACCESSORS
(__out == true ==> SEPEXISTS(d_prevDelim_p, d_token_p, p, (p ↦ _ && d_sharedData.inputType(*p) == SFT))) && (__out == false ==> SEPFORALL(d_prevDelim_p, d_token_p, p, (p ↦ _ && d_sharedData.inputType(*p) != SFT)))
