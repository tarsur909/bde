...
// MANIPULATORS
__out == *this && (d_endFlag == true || d_cursor_p != old_d_cursor_p) ⋆ (d_prevDelim_p ↦ _ ⋆ d_token_p ↦ _ ⋆ d_postDelim_p ↦ _ ⋆ d_cursor_p ↦ _)
...
// ACCESSORS
(__out == true ==> SEXISTS(d_prevDelim_p, d_token_p, p, (p |-> _ ⋆ SFT == d_sharedData.inputType(*p)))) && (__out == false ==> SFORALL(d_prevDelim_p, d_token_p, p, (p |-> _ ⋆ SFT != d_sharedData.inputType(*p))))
