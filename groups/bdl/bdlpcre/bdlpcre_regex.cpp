...
// PRIVATE ACCESSORS
(__out == 0 ==> (matchContextData->d_matchData_p != 0 ⋆ matchContextData->d_matchContext_p != 0 ⋆ (matchContextData->d_jitStack_p == 0 || (matchContextData->d_jitStack_p != 0)))) && (__out == k_INTERNAL_ERROR ==> true)
int
...
// ACCESSORS
(bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> __out == RegEx::k_STATUS_SUCCESS) && (!bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> __out != -1)
int
...
((__out == k_STATUS_SUCCESS) ==> (d_patternCode_p != nullptr)) && ((__out == k_INTERNAL_ERROR) ==> (d_patternCode_p == nullptr)) && (__out == k_STATUS_SUCCESS || __out == k_INTERNAL_ERROR)
