...
// PRIVATE ACCESSORS
// requires: matchContextData != 0 && d_pcre2Context_p != 0 && d_pcre2PatternCode_p != 0 && (d_jitStackSize >= 0 || d_jitStackSize == 0)
// ensures: (__out == 0 ==> (matchContextData->d_matchData_p != 0 ⋆ matchContextData->d_matchContext_p != 0 ⋆ (matchContextData->d_jitStack_p != 0 || d_jitStackSize == 0))) && (__out == k_INTERNAL_ERROR ==> (matchContextData->d_matchData_p == old_matchContextData->d_matchData_p ⋆ matchContextData->d_matchContext_p == old_matchContextData->d_matchContext_p ⋆ matchContextData->d_jitStack_p == old_matchContextData->d_jitStack_p))
int
...
// ensures: (bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> __out == RegEx::k_STATUS_SUCCESS) && (!bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> __out != -1)
int
