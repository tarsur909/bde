...
// MANIPULATORS
d_pcre2Context_p ↦ pcre2Context ⋆ d_pcre2PatternCode_p ↦ patternCode ⋆ d_depthLimit ↦ depthLimit ⋆ d_jitStackSize ↦ jitStackSize
...
// ACCESSORS
(bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> (__out == RegEx::k_STATUS_SUCCESS && *matchContextData == d_mainThreadMatchData)) && (!bslmt::ThreadUtil::isEqual(d_mainThread, bslmt::ThreadUtil::self()) ==> __out == allocateMatchContext(matchContextData))
int
...
(__out == k_STATUS_SUCCESS) ==> (errorMessage != 0 && errorOffset != 0) && (__out == k_INTERNAL_ERROR) ==> (errorMessage != 0 && errorOffset != 0)
