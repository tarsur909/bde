...
// ensures: (__out == true) ==> (x.value().d_raw & k_SIGN_MASK) && (__out == false) ==> !(x.value().d_raw & k_SIGN_MASK)
inline
...
/// Return `true` if the specified `x` is negative and `false` otherwise.
// ensures: ((x.value().d_raw.w[0] & k_SIGN_MASK) != 0 || (x.value().d_raw.w[1] & k_SIGN_MASK) != 0) ==> __out == true && ((x.value().d_raw.w[0] & k_SIGN_MASK) == 0 && (x.value().d_raw.w[1] & k_SIGN_MASK) == 0) ==> __out == false
inline
