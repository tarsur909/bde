...
// requires: true
// ensures: __out == pageSize.loadRelaxed() && (pageSize.loadRelaxed() != 0 ==> __out != 0)
...
// requires: size > 0 ==> SEPFORALL(0, size, i, (res_tmp + i) ↦ _)
// ensures: (__out == 0) || (__out != 0 ==> SEPFORALL(0, size, i, (__out + i) ↦ _))
...
// ensures: (__out == 0 ==> (address ↦ _)) && (__out != 0 ==> true)
...
/// `address` having the specified `pageSize` (in bytes).  The behavior is
/// undefined unless `pageSize == getSystemPageSize()`.
// requires: pageSize == getSystemPageSize() && address != 0
// ensures: (__out == 0) ==> (pageSize == getSystemPageSize() && address != 0)
...
// MANIPULATORS
// requires: size >= 0
// ensures: (size == 0 ==> __out == 0) && (size != 0 ==> (__out != 0 ⋆ (__out ↦ _ ⋆ (__out + size ↦ _))))
