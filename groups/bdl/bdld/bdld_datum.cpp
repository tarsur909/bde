...
// ACCESSORS
(__out == stream) && (__out.good() || __out.bad())
...
// ACCESSORS
(__out == stream) && (__out.bad() ==> stream.bad()) && (__out.good() ==> stream.good()) && (__out != stream ==> (__out ↦ _ ⋆ stream ↦ _))
...
// ACCESSORS
(__out == stream) && (stream.bad() ==> __out.bad()) && (stream.good() ==> __out.good()) && (__out->stream_state ↦ stream->stream_state ⋆ __out->buffer ↦ stream->buffer) && (__out->buffer ↦ bsl::flush)
                                   ...
                                   int           spacesPerLevel) const
{
    ...
    return stream << bsl::flush;
                          ...
                          // -----------------
// ACCESSORS
(__out == 0 ==> !findElementBinary(key, *this) && !findElementLinear(key, *this)) && (__out != 0 ==> true)
