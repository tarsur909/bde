...
// requires: length >= 0 && capacity >= 0 && length < bsl::numeric_limits<DatumArrayBuilder::SizeType>::max() / 2
// ensures: __out >= length && (capacity == 0 ==> __out == 1) && (capacity != 0 ==> EXISTS(0, 31, k, __out == capacity * (1 << k)))
