...
// ACCESSORS
(time.hour() < 24) ==> (__out == iter->d_code || __out == finalTransitionCode() || __out == d_initialTransitionCode)
...
// ACCESSORS
EXISTS(0, d_timetable.length(), i, d_timetable[i].size() != 0) ==> __out == Timetable_ConstIterator(*this, dayIndex, 0) && (dayIndex < d_timetable.length() ==> d_timetable[dayIndex].size() != 0)
...
// MANIPULATORS
(__out.d_transitionIndex == old_d_transitionIndex + 1 || (__out.d_dayIndex == old_d_dayIndex + 1 && __out.d_transitionIndex == 0)) && (__out.d_dayIndex < d_timetable_p->d_timetable.length()) && (__out.d_transitionIndex < d_timetable_p->d_timetable[__out.d_dayIndex].size())
