...
// ACCESSORS
// requires: d_timetable.size() >= 0 && SEPFORALL(0, d_timetable.size(), i, d_timetable[i].size() >= 0)
// ensures: (__out == Timetable_ConstIterator(*this, dayIndex, 0) && (dayIndex < d_timetable.length() && d_timetable[dayIndex].size() > 0)) || (dayIndex == d_timetable.length() && __out == Timetable_ConstIterator(*this, d_timetable.length(), 0))
...
// MANIPULATORS
// requires: d_dayIndex < d_timetable_p->d_timetable.length()
// ensures: __out == *this
