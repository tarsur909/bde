...
(__out == 0 ==> (*result ↦ tmp ⋆ *nextPos ↦ end)) && (__out == -1 ==> (*result ↦ old_result ⋆ *nextPos ↦ old_nextPos))
...
/// error.
(__out == 0 ==> *nextPos == begin + 8) && (__out == -1 ==> true)
...
(__out == 0 ==> ((*nextPos ↦ _ ⋆ *microsecond ↦ _) && SEPFORALL(begin, *nextPos, p, p ↦ _))) && (__out == -1 ==> (begin == end || !isdigit(*begin)))
...
/// error.
(__out == 0 ==> (*nextPos != 0 ⋆ *minuteOffset != 0)) && (__out == -1 ==> (*nextPos == old_nextPos ⋆ *minuteOffset == old_minuteOffset))
...
(__out == 0 ==> ((*nextPos ↦ _ ⋆ time->hour() ↦ _ ⋆ time->minute() ↦ _ ⋆ time->second() ↦ _ ⋆ time->millisecond() ↦ _ ⋆ time->microsecond() ↦ _ ⋆ tzOffset ↦ _ ⋆ isNextDay ↦ _))) && (__out == -1 ==> true)
...
__out >= 0 && (0 == tzOffset && configuration.useZAbbreviationForUtc() ==> __out == 1) && (0 != tzOffset ==> __out > 1)
