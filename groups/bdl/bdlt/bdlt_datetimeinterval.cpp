...
// HELPER FUNCTIONS
// requires: result != NULL && spec != NULL && numBytes >= 0
// ensures: __out >= 0
...
// CLASS METHODS
// requires: (u::k_HOURS_CEILING >= hours && u::k_MINUTES_CEILING >= minutes && u::k_SECONDS_CEILING >= seconds && u::k_MILLISECONDS_CEILING >= milliseconds && u::k_HOURS_FLOOR <= hours && u::k_MINUTES_FLOOR <= minutes && u::k_SECONDS_FLOOR <= seconds && u::k_MILLISECONDS_FLOOR <= milliseconds)
// ensures: (__out == true) ==> (hours == old_hours % TimeUnitRatio::k_H_PER_D && minutes == old_minutes % TimeUnitRatio::k_M_PER_D && seconds == old_seconds % TimeUnitRatio::k_S_PER_D && milliseconds == old_milliseconds % TimeUnitRatio::k_MS_PER_D)
