...
// MANIPULATORS
(timetableName != nullptr ==> __out != nullptr && __out.use_count() > 0) || (timetableName == nullptr ==> __out == nullptr && __out.use_count() == 0)
...
// ACCESSORS
(__out.use_count() > 0) || (__out == bsl::shared_ptr<const Timetable>())
bsl::shared_ptr<const Timetable>
