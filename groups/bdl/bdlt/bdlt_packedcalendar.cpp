...
/// `lastDate < firstDate`.
(firstDate <= lastDate ==> __out >= 0) && (firstDate > lastDate ==> __out == 0)
static
...
(offset == d_holidayOffsets[__out] ==> __out >= 0 && __out <= static_cast<int>(d_holidayOffsets.length() - 1)) && (offset != d_holidayOffsets[__out] ==> false)
