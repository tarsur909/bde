...
/// `lastDate < firstDate`.
// requires: true
// ensures: __out == ((firstDate <= lastDate ? (lastDate - firstDate + 1) : 0) / 7) * weekendDays.length() + (reinterpret_cast<const DayOfWeekSet&>(s_partialWeeks[static_cast<int>(firstDate.dayOfWeek())][(firstDate <= lastDate ? (lastDate - firstDate + 1) : 0) % 7]) & weekendDays).length()
static
...
// requires: offset >= 0
// ensures: __out >= 0 && __out < static_cast<int>(d_holidayOffsets.length()) && d_holidayOffsets[__out] == offset && (d_holidayCodesIndex.length() == d_holidayOffsets.length())
...
// ensures: (__out == d_holidayCodes.begin() + d_holidayCodesIndex[i - offsetBegin]) || (__out == d_holidayCodes.begin() + d_holidayCodes.length())
...
// MANIPULATORS
// requires: rhs.d_offsetIter ↦ _ ⋆ rhs.d_calendar_p ↦ _ ⋆ rhs.d_currentOffset ↦ _ ⋆ rhs.d_endFlag ↦ _
