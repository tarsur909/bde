...
/// `lastDate < firstDate`.
__out >= 0 && (firstDate <= lastDate ==> __out == ((lastDate - firstDate + 1) / 7) * weekendDays.length() + ((myPartialWeek & weekendDays).length()))
static
...
// ACCESSORS
(__out == d_holidayCodes.end() ==> !isHoliday(date)) && (__out != d_holidayCodes.end() ==> isHoliday(date))
