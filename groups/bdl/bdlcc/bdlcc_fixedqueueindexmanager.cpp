...
// ensures: (__out == true ==> (encodedPushIndex & k_DISABLED_STATE_MASK) != 0) && (__out == false ==> (encodedPushIndex & k_DISABLED_STATE_MASK) == 0)
inline
...
// ACCESSORS
// ensures: (__out == 0) || (__out > 0 && __out <= d_capacity) ==> (__out == 0 || (__out > 0 && __out <= d_capacity))
