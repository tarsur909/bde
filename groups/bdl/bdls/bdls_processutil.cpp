...
// requires: strlen(path) > 0
// ensures: (__out == true) ==> (!(s.st_mode & S_IFDIR) && (s.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH))) && (__out == false) ==> ((s.st_mode & S_IFDIR) || !(s.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)))
static inline
