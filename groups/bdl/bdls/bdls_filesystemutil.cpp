...
// requires: idx < str.size() && (len == bsl::string::npos || idx + len <= str.size())
// ensures: __out.data() == &str[idx] && __out.size() == (len == bsl::string::npos ? str.size() - idx : len)
...
// requires: path != nullptr && strlen(path) < 3
// ensures: (__out == true ==> ((*path == '.' && !path[1]) || (*path == '.' && path[1] == '.' && !path[2]))) && (__out == false ==> !(*path == '.' && !path[1]) && !(*path == '.' && path[1] == '.' && !path[2]))
static inline
...
// requires: path != nullptr && strlen(path) >= 1 && strlen(path) <= 3 && SEPFORALL(0, strlen(path), i, path + i ↦ _)
// ensures: (__out == true ==> (SEPFORALL(0, strlen(path), i, (i == 0 || i == 1 || i == 2) && (path[i] == '.' || (i == 1 && path[i] == '/') || (i == 2 && path[i] == '/')))) && (strlen(path) == 1 || strlen(path) == 2 || strlen(path) == 3)) && (__out == false ==> !(SEPFORALL(0, strlen(path), i, (i == 0 || i == 1 || i == 2) && (path[i] == '.' || (i == 1 && path[i] == '/') || (i == 2 && path[i] == '/')))) || !(strlen(path) == 1 || strlen(path) == 2 || strlen(path) == 3))
static inline
...
// requires: BloombergLP::bdls::FilesystemUtil::isValidDirectoryFD(dirFD)
// ensures: true
static
