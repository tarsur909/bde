...
// ACCESSORS
// requires: true
// ensures: (__out == 0 ==> (fstat(d_fileId, &buf) != 0 || buf.st_size <= 0)) && (__out > 0 ==> (fstat(d_fileId, &buf) == 0 && buf.st_size > 0))
bsl::streamoff
...
// requires: true
// ensures: (__out == traits_type::to_int_type(*gptr()) || __out == traits_type::eof() || __out == underflowRead())
