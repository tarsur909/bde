...
// requires: maxNestedDepth >= 0 && tokenizer != nullptr && error != nullptr && result != nullptr
...
// requires: tokenizer != 0 && result != 0 && error != 0 && maxNestedDepth >= 0
...
// ensures: (__out == 0 ==> (result->type() == JsonType::e_NULL || result->type() == JsonType::e_BOOLEAN || result->type() == JsonType::e_STRING || result->type() == JsonType::e_NUMBER)) && (__out != 0 ==> error != nullptr)
...
class JsonObjectSortedMemberIterator {
  ...
  public:
    ...
    // MANIPULATORS
    // requires: d_it != d_sortedMembers.end() && (d_it == d_sortedMembers.begin() || EXISTS(d_sortedMembers.begin(), d_sortedMembers.end(), it, it == d_it))
...
// CLASS METHODS
// requires: result && errorDescription && input
// ensures: (__out == 0 ==> (result != nullptr)) && (__out != 0 ==> errorDescription != nullptr)
