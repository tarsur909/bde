...
(__out == 0) || (__out != 0 ==> error != nullptr && error->message().size() > 0)
...
(__out == 0) ==> (result->type() == JsonType::e_NULL || result->type() == JsonType::e_BOOLEAN || result->type() == JsonType::e_STRING || result->type() == JsonType::e_NUMBER)
