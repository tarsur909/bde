...
// requires: maxNestedDepth >= 0 && tokenizer != nullptr && error != nullptr
...
// requires: maxNestedDepth >= 0 && error != nullptr
// ensures: (__out == 0 ==> true) && (__out != 0 ==> error != nullptr)
...
// requires: tokenizer != NULL && result != NULL && (res_tmp == 0 ==> (result->type() == JsonType::e_NULL || result->type() == JsonType::e_BOOLEAN || result->type() == JsonType::e_STRING || result->type() == JsonType::e_NUMBER)) && (res_tmp == -1 ==> error != NULL)
// ensures: (__out == 0 ==> (result->type() == JsonType::e_NULL || result->type() == JsonType::e_BOOLEAN || result->type() == JsonType::e_STRING || result->type() == JsonType::e_NUMBER)) && (__out == -1 ==> error != nullptr)
...
bool isValid() const
    ...
    // requires: d_it != nullptr && d_it->first ↦ _ ⋆ d_it->second ↦ _
// ensures: __out == *d_it
...
// ensures: (__out == false ==> d_it != d_sortedMembers.end()) && (__out == true ==> d_it == d_sortedMembers.end())
bool next()
        // Advance to the next member in the object.  Return 'true' if the new
        // position is valid.
    {
    ...
    // ACCESSORS
    // requires: true
// ensures: __out == (d_it == d_sortedMembers.begin())
bool isFirst() const
        // Return 'true' if this object is in its initial state.
    {
...
// CLASS METHODS
// requires: result != 0 && errorDescription != 0 && input != 0
// ensures: (__out == 0) || (__out != 0 && errorDescription->message() != "")
