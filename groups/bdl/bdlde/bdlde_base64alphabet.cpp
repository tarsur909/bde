...
// CLASS METHODS
// requires: stream.good() && (SEPFORALL(0, strlen(Base64Alphabet::toAscii(value)), i, EXISTS(j, strlen(Base64Alphabet::toAscii(value)), stream + i ↦ Base64Alphabet::toAscii(value)[j])))
// ensures: __out == stream && (stream.bad() || (stream.good() && (SEPFORALL(0, strlen(Base64Alphabet::toAscii(value)), i, stream + i ↦ Base64Alphabet::toAscii(value)[i]))))
