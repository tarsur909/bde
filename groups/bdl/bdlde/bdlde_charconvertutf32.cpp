...
// ensures: (__out == true) ==> !(oct & k_ONE_OCTET_MASK) && (__out == false) ==> (oct & k_ONE_OCTET_MASK)
static inline
...
/// UTF-8 sequence and `false` otherwise.
// ensures: __out == ((oct & k_THREE_OCTET_MASK) == k_THREE_OCTET_TAG)
static inline
...
/// pointed to by the specified `octBuf`.
// requires: octBuf != nullptr && SEPFORALL(0, 3, i, octBuf + i ↦ _)
...
// requires: uc < (1u << (k_TWO_OCT_CONT_WID + k_CONTINUE_CONT_WID))
...
/// UTF-8 octets or less and `false` otherwise.
// ensures: ((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) == 0) ==> __out == true && ((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) != 0) ==> __out == false
static inline
