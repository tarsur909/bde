...
(__out == true ==> !(oct & k_ONE_OCTET_MASK)) && (__out == false ==> (oct & k_ONE_OCTET_MASK) != 0)
static inline
...
(octBuf[0] ↦ _ ⋆ octBuf[1] ↦ _) && (__out == ((octBuf[1] & ~k_CONTINUE_MASK) | ((octBuf[0] & ~k_TWO_OCTET_MASK) << k_CONTINUE_CONT_WID)))
static inline
...
(__out >= 0) && (__out <= n) && (__out == n || !isContinuation(octBuf[__out])) && FORALL(0, __out, i, isContinuation(octBuf[i]))
static inline
...
(__out == true ==> (uc & (~0 << k_ONE_OCT_CONT_WID)) == 0) && (__out == false ==> (uc & (~0 << k_ONE_OCT_CONT_WID)) != 0)
static inline
...
(__out == true ==> (uc & (~0 << (k_TWO_OCT_CONT_WID + k_CONTINUE_CONT_WID))) == 0) && (__out == false ==> (uc & (~0 << (k_TWO_OCT_CONT_WID + k_CONTINUE_CONT_WID))) != 0)
static inline
...
/// UTF-8 octets or less and `false` otherwise.
(__out == true ==> ((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) == 0)) && (__out == false ==> ((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) != 0))
static inline
