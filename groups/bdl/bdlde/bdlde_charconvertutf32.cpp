...
SEPFORALL(0, __out, i, (octBuf + i ↦ _) && isContinuation(octBuf[i])) && (__out == n || !isContinuation(octBuf[__out]))
static inline
...
/// UTF-8 octets or less and `false` otherwise.
((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) == 0) ==> __out == true && ((uc & (~0 << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID))) != 0) ==> __out == false
static inline
...
(isSingleOctet(*input) ==> __out == input + 1) && (!isSingleOctet(*input) ==> __out == input + 1 + lookaheadContinuations(input + 1, expected))
static inline
