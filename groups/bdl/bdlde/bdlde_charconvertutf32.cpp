...
// requires: SEPFORALL(0, 2, i, octBuf + i ↦ _)
...
// requires: SEPFORALL(0, n, i, (octBuf + i ↦ _))
// ensures: SEPFORALL(0, __out, i, (octBuf + i ↦ sep_v && isContinuation(sep_v))) && (__out == n || !isContinuation(*(octBuf + __out)))
static inline
...
/// UTF-8 octets or less and `false` otherwise.
// requires: true
// ensures: (__out == true) == ((uc & (~ (unsigned int) 0 << (k_FOUR_OCT_CONT_WID + 3 * k_CONTINUE_CONT_WID))) == 0)
static inline
...
// requires: input != 0 && (input ↦ _ ⋆ (input + 1 ↦ _) ⋆ (input + 2 ↦ _) ⋆ (input + 3 ↦ _))
// ensures: (isSingleOctet(*input) ==> __out == input + 1) && (isTwoOctetHeader(*input) ==> __out == input + 2) && (isThreeOctetHeader(*input) ==> __out == input + 3) && (isFourOctetHeader(*input) ==> __out == input + 4) && (input ↦ _ ⋆ (input + 1 ↦ _) ⋆ (input + 2 ↦ _) ⋆ (input + 3 ↦ _))
static inline
