...
// FREE OPERATORS
(__out == true ==> (lhs.d_length == rhs.d_length && SFORALL(0, getLengthInUse(lhs.d_length), i, (lhs.d_buffer + i ↦ rhs.d_buffer[i])))) && (__out == false ==> (lhs.d_length != rhs.d_length || 0 != bsl::memcmp(lhs.d_buffer, rhs.d_buffer, (sizeof *lhs.d_buffer) * getLengthInUse(lhs.d_length))))
