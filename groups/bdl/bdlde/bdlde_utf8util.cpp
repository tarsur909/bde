...
/// sequence describing a single valid code point.
(pc ↦ _ ⋆ (pc+1) ↦ _ ⋆ (pc+2) ↦ _) && (__out == ((*pc & 0xf) << 12) | ((pc[1] & k_CONT_VALUE_MASK) << 6) | (pc[2] & k_CONT_VALUE_MASK))
inline
...
(__out >= 0 ==> true) && (__out < 0 ==> (*invalidString != nullptr))
...
(__out >= 0) ==> (SFORALL(0, length, i, (string + i |-> _)))
