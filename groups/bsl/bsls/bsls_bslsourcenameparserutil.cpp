...
__out == (FORALL(begin, end, i, *i == ch) ==> count <= 0)
static
inline
...
(sourceType & BslSourceNameParserUtil::k_MASK_TEST) != 0 ==> __out == true && (sourceType & BslSourceNameParserUtil::k_MASK_TEST) == 0 ==> __out == false
static
inline
...
FORALL(0, strlen(__out), i, strpbrk(&__out[i], pathSeparators) == nullptr)
static
...
// CLASS METHODS
(__out == 0 ==> (componentNamePtr != NULL && componentNameLength != NULL && *componentNameLength > 0)) && (__out != 0 ==> (__out == -1 || __out == -2 || __out == -3))
