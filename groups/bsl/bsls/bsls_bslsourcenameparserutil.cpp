...
/// than `end`.
(__out != begin ==> SEPEXISTS(begin, end, i, (i ↦ ch) && (__out == i + 1))) && (__out == begin ==> SEPFORALL(begin, end, i, (i ↦ _) && (i ↦ _ != ch)))
static
inline
...
FORALL(0, strlen(__out), i, strchr(pathSeparators, __out[i]) == NULL)
static
...
// CLASS METHODS
(__out == 0 ==> componentNamePtr != NULL && componentNameLength != NULL) && (__out != 0 ==> __out == -1 || __out == -2 || __out == -3)
