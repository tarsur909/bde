...
// requires: true
// ensures: __out == (0 != (sourceType & BslSourceNameParserUtil::k_MASK_TEST))
static
inline
...
// requires: filename != nullptr && strlen(filename) >= 0
// ensures: (__out >= filename) && (__out <= filename + strlen(filename)) && (strpbrk(__out, pathSeparators) == nullptr)
static
