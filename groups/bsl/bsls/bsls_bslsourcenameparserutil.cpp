...
(__out == true ==> EXISTS(begin, end, i, *(begin + i) == ch && count - 1 == 0)) && (__out == false ==> FORALL(begin, end, i, *(begin + i) == ch ==> count > 0))
static
inline
...
/// than `end`.
(__out != begin ==> (*(__out - 1) == ch)) && (__out == begin ==> FORALL(begin, end, p, *p != ch))
static
inline
