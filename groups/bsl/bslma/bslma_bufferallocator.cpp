...
// requires: cursor != nullptr && buffer != nullptr && 0 < alignment && alignment <= bsls::AlignmentUtil::BSLS_MAX_ALIGNMENT && (alignment & (alignment - 1)) == 0
// ensures: (__out == nullptr && *cursor + bsls::AlignmentUtil::calculateAlignmentOffset(buffer + *cursor, alignment) + size > bufSize) || (__out != nullptr && __out >= buffer && __out + size <= buffer + bufSize)
static
...
// MANIPULATORS
// requires: true
// ensures: (size == 0 ==> __out == nullptr) && (size != 0 ==> __out != nullptr || __out == nullptr)
