...
// requires: true
// ensures: (__out == true) ==> (!node || node->isBlack())
     ...
     // constraints is localized to 'node').
{
    ...
    if (node) {
        node->makeBlack();
                        ...
                        // ----------------
// CLASS METHODS
// requires: subtree != 0 && SEPFORALL(0, n, i, (subtree + i)->leftChild() != 0) && EXISTS(0, n, i, (subtree + i)->leftChild() == 0)
// ensures: __out != 0 && !(__out->leftChild())
