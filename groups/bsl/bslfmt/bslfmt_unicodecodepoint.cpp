...
/// return the result.
// ensures: maxBytes < 2 ==> __out == false
inline
...
// ensures: __out == ((*pc & 0x1f) << 6) | ((pc[1] & 0x3f))
inline
...
// ensures: (pc ↦ _ ⋆ (pc + 1 ↦ _ ⋆ (pc + 2 ↦ _))) && (__out == ((*pc & 0xf) << 12) | ((pc[1] & k_UTF8_CONT_VALUE_MASK) << 6) | (pc[2] & k_UTF8_CONT_VALUE_MASK))
inline
...
/// describing a single valid code point.
// requires: pc != nullptr && pc + 3 < reinterpret_cast<unsigned char*>(std::numeric_limits<std::size_t>::max())
inline
...
// ensures: (codepoint < UnicodeData::s_doubleFieldWidthRanges->d_start ==> __out == 1) && (codepoint >= UnicodeData::s_doubleFieldWidthRanges->d_start ==> __out == 1 || __out == 2)
inline
...
/// and `false` otherwise.
// ensures: (__out == true) ==> ((value & 0xc0) != 0x80)
inline
...
/// otherwise.
// ensures: __out == ((k_UTF16_SURROGATE_MASK_TESTBOTH & value) == k_UTF16_HIGH_SURROGATE_START)
inline
