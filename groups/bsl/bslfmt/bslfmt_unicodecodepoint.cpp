...
/// return the result.
// requires: maxBytes >= 2
// ensures: (maxBytes < 2 ==> __out == false) && (__out == true ==> (maxBytes >= 2))
inline
...
// ensures: __out == ((*pc & 0xf) << 12) | ((pc[1] & k_UTF8_CONT_VALUE_MASK) << 6) | (pc[2] & k_UTF8_CONT_VALUE_MASK) && (pc ↦ _ ⋆ (pc + 1 ↦ _ ⋆ (pc + 2 ↦ _)))
inline
