...
/// describing a single valid code point.
// requires: pc != NULL && strlen((const char*)pc) >= 4
// ensures: true
inline
...
// requires: (codepoint < UnicodeData::s_doubleFieldWidthRanges->d_start) ==> res_tmp == 1 && (codepoint >= UnicodeData::s_doubleFieldWidthRanges->d_start) ==> res_tmp == 2
// ensures: (codepoint < UnicodeData::s_doubleFieldWidthRanges->d_start) ==> __out == 1 ⋆ (codepoint >= UnicodeData::s_doubleFieldWidthRanges->d_start) ==> __out == 2
inline
...
/// `false` otherwise.
// requires: true
// ensures: __out == ((k_UTF16_SURROGATE_MASK_TESTONE & value) == k_UTF16_HIGH_SURROGATE_START)
inline
