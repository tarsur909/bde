...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
(maxNestedDepth < 0 ==> __out == -4) && (tokenizer->tokenType() == baljsn::Tokenizer::e_ERROR ==> __out == -1) && EXISTS(0, maxNestedDepth, depth, decodeValue(&elementValue, errorStream, tokenizer, depth) != 0 ==> __out == -2) && (__out == 0 ==> maxNestedDepth >= 0 && tokenizer->tokenType() != baljsn::Tokenizer::e_ERROR)
