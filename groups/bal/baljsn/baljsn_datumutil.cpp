...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
// requires: (maxNestedDepth >= 0) || (res_tmp == -4)
// ensures: (maxNestedDepth < 0 ==> __out == -4) && (__out == -4 || __out == -2 || __out == -1 || __out == 0)
...
// requires: formatter != 0 && strictTypesCheckStatus != 0 && (name == 0 || name != 0)
// ensures: (__out == 0) ==> (FORALL(0, datum.length(), i, u::encodeValue(formatter, datum[i], strictTypesCheckStatus) == 0)) && (__out != 0 ==> EXISTS(0, datum.length(), i, __out == u::encodeValue(formatter, datum[i], strictTypesCheckStatus)))
...
/// value if `datum` cannot be encoded, which should stop further encoding.
// requires: (formatter != nullptr) && (datum.size() >= 0) && (strictTypesCheckStatus != nullptr) && (name == nullptr || name->size() >= 0)
