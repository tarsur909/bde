...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
(__out == 0) ==> (tokenizer->tokenType() == baljsn::Tokenizer::e_END_ARRAY) && (__out != 0) ==> (tokenizer->tokenType() != baljsn::Tokenizer::e_END_ARRAY)
...
/// value if `datum` cannot be encoded, which should stop further encoding.
(__out == 0 ==> SEPFORALL(0, datum.size(), i, u::encodeValue(formatter, datum[i].value(), strictTypesCheckStatus, &datum[i].key()) == 0)) && (__out != 0 ==> SEPEXISTS(0, datum.size(), i, u::encodeValue(formatter, datum[i].value(), strictTypesCheckStatus, &datum[i].key()) == __out))
