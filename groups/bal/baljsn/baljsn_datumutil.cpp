...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
// requires: maxNestedDepth >= 0 && result != NULL && tokenizer != NULL
...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
// requires: (maxNestedDepth >= 0) && (tokenizer->tokenType() != baljsn::Tokenizer::e_ERROR)
// ensures: (maxNestedDepth < 0 ==> __out == -4) && (__out == 0 || __out == -1 || __out == -2 || __out == -4)
...
// requires: formatter != 0 && datum.length() >= 0 && strictTypesCheckStatus != 0 && (name == 0 || name->size() >= 0)
// ensures: __out == 0 || EXISTS(0, datum.length(), i, __out == u::encodeValue(formatter, datum[i], strictTypesCheckStatus))
...
/// value if `datum` cannot be encoded, which should stop further encoding.
// requires: formatter != nullptr && datum != nullptr && strictTypesCheckStatus != nullptr && (name == nullptr || name->data() != nullptr)
