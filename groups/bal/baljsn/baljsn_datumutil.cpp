...
// requires: FORALL(0, datum.length(), i, u::encodeValue(formatter, datum[i], strictTypesCheckStatus) == 0)
// ensures: (__out == 0) ==> FORALL(0, datum.length(), i, u::encodeValue(formatter, datum[i], strictTypesCheckStatus) == 0)
...
/// value if `datum` cannot be encoded, which should stop further encoding.
// requires: formatter != 0 && datum.size() >= 0 && strictTypesCheckStatus != 0 && (name == 0 || name != 0)
