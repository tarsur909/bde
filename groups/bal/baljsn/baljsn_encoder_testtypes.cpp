...
// requires: true
// ensures: (__out == *this) && (this != &rhs ==> (d_selectionId == rhs.d_selectionId ⋆ d_selection0.object() == rhs.d_selection0.object()))
...
// requires: (name != nullptr) && (strlen(name) == nameLength)
...
// requires: d_selectionId == SELECTION_ID_SELECTION0 || d_selectionId == SELECTION_ID_UNDEFINED
// ensures: (__out == SELECTION_INFO_ARRAY[SELECTION_INDEX_SELECTION0].name()) || (__out == "(* UNDEFINED *)")
...
// requires: strlen(name) == nameLength
...
// requires: true
// ensures: (d_selectionId == SELECTION_ID_SELECTION0) && (d_selection0.object() ↦ value) && (__out ↦ value)
...
// requires: true
// ensures: __out == *this && (this != &rhs ==> __out.d_selectionId == rhs.d_selectionId)
...
// requires: true
// ensures: (d_selectionId == SELECTION_ID_SIMPLE ==> (__out ↦ value)) && (d_selectionId != SELECTION_ID_SIMPLE ==> (__out ↦ value))
...
// requires: true
// ensures: (this != &rhs) ==> ((rhs.d_selectionId == SELECTION_ID_SEQUENCE) ==> (d_selectionId == SELECTION_ID_SEQUENCE ⋆ d_sequence.object() == rhs.d_sequence.object()) ⋆ (rhs.d_selectionId == SELECTION_ID_UNDEFINED) ==> (d_selectionId == SELECTION_ID_UNDEFINED))
...
// requires: name != 0 && nameLength == strlen(name)
// ensures: (lookupSelectionInfo(name, nameLength) == 0 ==> __out == -1) && (lookupSelectionInfo(name, nameLength) != 0 ==> __out == makeSelection(lookupSelectionInfo(name, nameLength)->d_id))
