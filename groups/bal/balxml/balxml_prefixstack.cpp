...
// requires: true
// ensures: (__out != nullPrefix ==> EXISTS(0, ARRAY_LEN(predefinedPrefixes), i, prefix == predefinedPrefixes[i].d_prefix)) && (__out == nullPrefix ==> FORALL(0, ARRAY_LEN(predefinedPrefixes), i, prefix != predefinedPrefixes[i].d_prefix))
...
// MANIPULATORS
// requires: true
// ensures: (namespaceUri.empty() ==> __out == -1) && (!namespaceUri.empty() ==> __out != -1)
...
// ACCESSORS
// requires: true
// ensures: (SEPEXISTS(0, d_numPrefixes, i, (d_prefixes[i].first == prefix) && (__out == d_prefixes[i].second))) || (SEPFORALL(0, d_numPrefixes, i, (d_prefixes[i].first != prefix)) && (__out == lookupPredefinedPrefix(prefix).d_nsid))
int
