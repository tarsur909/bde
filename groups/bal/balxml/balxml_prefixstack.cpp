...
(FORALL(0, ARRAY_LEN(predefinedPrefixes), i, prefix != predefinedPrefixes[i].d_prefix) ==> &__out == &nullPrefix) && (EXISTS(0, ARRAY_LEN(predefinedPrefixes), i, prefix == predefinedPrefixes[i].d_prefix) ==> __out.d_prefix == prefix)
...
// MANIPULATORS
__out == nsId && (namespaceUri.empty() ==> __out == -1)
