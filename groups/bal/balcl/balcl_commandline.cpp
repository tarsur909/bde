...
// requires: true
// ensures: (environmentVariableName.empty() ==> !__out) && (__out ==> !environmentVariableName.empty())
...
// ensures: (__out == *this && &rhs == this) || (__out.d_options == rhs.d_options && (__out.d_state == e_NOT_PARSED || (__out.d_state == e_PARSED && __out.d_arguments == rhs.d_arguments)))
...
// requires: EXISTS(0, d_options.size(), i, d_options[i].name() == name && d_options[i].argType() == OptionInfo::e_FLAG)
// ensures: true
...
// ACCESSORS
// requires: true
// ensures: (__out != -1 ==> (d_schema_p->cbegin() + __out)->d_name_p == name) && (__out == -1 ==> true)
