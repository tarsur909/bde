...
(__out >= 0) && (__out == 0 ==> FORALL(0, options.size(), i, (options[i]->isTagValid() && options[i]->isNameValid() && options[i]->isDescriptionValid())))
...
(__out == -1) ==> (errorStream != old_errorStream)
...
// PRIVATE ACCESSORS
(__out != -1 ==> (0 <= __out && __out < d_options.size() && d_options[__out].name() == name)) && (__out == -1 ==> FORALL(0, d_options.size(), i, d_options[i].name() != name))
...
// CLASS METHODS
(__out == true ==> status == 0) && (__out == false ==> status != 0)
...
// MANIPULATORS
(__out == *this) && (d_state == e_NOT_PARSED || d_state == e_PARSED) && (d_options == rhs.d_options) && (d_state == e_PARSED ==> d_arguments == rhs.d_arguments)
...
// ACCESSORS
(__out != -1 ==> EXISTS(d_schema_p->cbegin(), d_schema_p->cend(), itr, (itr->d_name_p == name) && (__out == static_cast<int>(bsl::distance(d_schema_p->begin(), itr))))) && (__out == -1 ==> FORALL(d_schema_p->cbegin(), d_schema_p->cend(), itr, itr->d_name_p != name))
