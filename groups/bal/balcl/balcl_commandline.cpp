...
// CLASS METHODS
(__out == true ==> (0 == u::validate(options, errorStream))) && (__out == false ==> (0 != u::validate(options, errorStream)))
...
// MANIPULATORS
(__out.d_state == rhs.d_state) && (__out.d_options == rhs.d_options) && ((__out.d_state == e_PARSED) ==> (__out.d_arguments == rhs.d_arguments))
...
// ACCESSORS
(__out != -1 ==> EXISTS(d_schema_p->cbegin(), d_schema_p->cend(), itr, (itr->d_name_p == name) && (__out == static_cast<int>(bsl::distance(d_schema_p->cbegin(), itr))))) && (__out == -1 ==> FORALL(d_schema_p->cbegin(), d_schema_p->cend(), itr, itr->d_name_p != name))
