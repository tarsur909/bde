...
// ensures: (__out == units) && (*fractionalUnitDrainedInNanoUnits ↦ nanounits % k_NANOUNITS_PER_UNIT)
...
// MANIPULATORS
// requires: true
// ensures: (d_unitsInBucket + d_unitsReserved < d_capacity ==> __out == bsls::TimeInterval(0, 0)) && (d_unitsInBucket + d_unitsReserved >= d_capacity ==> __out >= bsls::TimeInterval(0, 0))
