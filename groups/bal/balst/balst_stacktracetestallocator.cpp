...
// PRIVATE ACCESSORS
(k_ALLOCATED_BLOCK_MAGIC == blockHdr->d_magic && this == blockHdr->d_allocator_p && d_blocks != 0 && blockHdr->d_prevNext_p != 0 && *blockHdr->d_prevNext_p != 0 && (blockHdr->d_next_p == 0 || k_ALLOCATED_BLOCK_MAGIC == blockHdr->d_next_p->d_magic) ==> __out == 0) && (k_ALLOCATED_BLOCK_MAGIC != blockHdr->d_magic || this != blockHdr->d_allocator_p || d_blocks == 0 || blockHdr->d_prevNext_p == 0 || *blockHdr->d_prevNext_p == 0 || (blockHdr->d_next_p != 0 && k_ALLOCATED_BLOCK_MAGIC != blockHdr->d_next_p->d_magic) ==> __out == -1)
