...
(EXISTS(0, strlen(logFilePattern), i, (logFilePattern[i] == '%' && (logFilePattern[i+1] == 'Y' || logFilePattern[i+1] == 'M' || logFilePattern[i+1] == 'D' || logFilePattern[i+1] == 'h' || logFilePattern[i+1] == 'm' || logFilePattern[i+1] == 's' || logFilePattern[i+1] == '%'))) ==> __out == true) && (FORALL(0, strlen(logFilePattern), i, logFilePattern[i] != '%') ==> __out == false)
...
(__out == 0 || __out == -1) && (__out == 0 ==> stream->rdbuf() != NULL)
...
(fuzzyEqual(referenceStartTime, fileCreationTimeUtc, interval) ==> __out == fileCreationTimeUtc + interval) && (!fuzzyEqual(referenceStartTime, fileCreationTimeUtc, interval) ==> __out >= fileCreationTimeUtc)
