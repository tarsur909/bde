...
// requires: logFilePattern != 0 && SEPFORALL(0, strlen(logFilePattern), i, (logFilePattern + i ↦ _))
// ensures: (__out == true ==> SEPEXISTS(0, strlen(logFilePattern), i, ((logFilePattern + i ↦ '%' ⋆ logFilePattern + i + 1 ↦ ('Y' || 'M' || 'D' || 'h' || 'm' || 's' || '%'))))) && (__out == false ==> SEPFORALL(0, strlen(logFilePattern), i, (logFilePattern + i ↦ sep_v && !(sep_v == '%' && SEPEXISTS(1, 1, j, (logFilePattern + i + j ↦ ('Y' || 'M' || 'D' || 'h' || 'm' || 's' || '%')))))))
...
// requires: stream != 0 && filename != 0 && stream->rdbuf() != 0 && dynamic_cast<bdls::FdStreamBuf*>(stream->rdbuf()) != 0
// ensures: (__out == 0 ==> (stream->rdbuf() != 0)) && (__out == -1 ==> true)
