...
// FREE OPERATORS
// requires: true
// ensures: (__out == true ==> (lhs.numRules() == rhs.numRules() && FORALL(0, lhs.numRules(), i, EXISTS(0, rhs.numRules(), j, lhs.getRuleById(i) == rhs.getRuleById(j))))) && (__out == false ==> (lhs.numRules() != rhs.numRules() || EXISTS(0, lhs.numRules(), i, 0 > rhs.ruleId(*lhs.getRuleById(i)))))
