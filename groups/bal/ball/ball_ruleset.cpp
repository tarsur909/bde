...
// ACCESSORS
(__out == -1) ==> (d_ruleHashtable.find(value) == d_ruleHashtable.end()) && (__out != -1) ==> EXISTS(0, d_ruleAddresses.size(), i, d_ruleAddresses[i] == &*d_ruleHashtable.find(value))
...
// FREE OPERATORS
__out == true ==> (lhs.numRules() == rhs.numRules() && FORALL(0, lhs.numRules(), i, rhs.ruleId(*lhs.getRuleById(i)) >= 0))
