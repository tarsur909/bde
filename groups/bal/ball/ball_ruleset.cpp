...
// FREE OPERATORS
// requires: true
// ensures: (__out == true ==> (lhs.numRules() == rhs.numRules() && FORALL(0, lhs.numRules(), i, rhs.ruleId(*lhs.getRuleById(i)) >= 0))) && (__out == false ==> (lhs.numRules() != rhs.numRules() || EXISTS(0, lhs.numRules(), i, lhs.getRuleById(i) != nullptr && rhs.ruleId(*lhs.getRuleById(i)) < 0)))
