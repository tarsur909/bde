...
// ACCESSORS
// ensures: (__out != -1 ==> SEPEXISTS(0, d_ruleAddresses.size(), i, d_ruleAddresses[i] == &*d_ruleHashtable.find(value) && __out == i)) && (__out == -1 ==> (d_ruleHashtable.find(value) == d_ruleHashtable.end() || SEPFORALL(0, d_ruleAddresses.size(), i, d_ruleAddresses[i] != &*d_ruleHashtable.find(value))))
...
// FREE OPERATORS
// ensures: (__out == true ==> lhs.numRules() == rhs.numRules() && FORALL(0, lhs.maxNumRules(), i, lhs.getRuleById(i) == nullptr || rhs.ruleId(*lhs.getRuleById(i)) >= 0)) && (__out == false ==> lhs.numRules() != rhs.numRules() || EXISTS(0, lhs.maxNumRules(), i, lhs.getRuleById(i) != nullptr && rhs.ruleId(*lhs.getRuleById(i)) < 0))
