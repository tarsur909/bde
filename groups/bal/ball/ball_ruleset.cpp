...
// ACCESSORS
// requires: true
// ensures: (__out == -1 ==> (d_ruleHashtable.find(value) == d_ruleHashtable.end() || std::find(d_ruleAddresses.begin(), d_ruleAddresses.end(), &*d_ruleHashtable.find(value)) == d_ruleAddresses.end())) && (__out != -1 ==> (d_ruleHashtable.find(value) != d_ruleHashtable.end() && d_ruleAddresses[__out] == &*d_ruleHashtable.find(value)))
...
// FREE OPERATORS
// requires: true
// ensures: (__out == true ==> (lhs.numRules() == rhs.numRules() && FORALL(0, lhs.numRules(), i, rhs.ruleId(*lhs.getRuleById(i)) >= 0))) && (__out == false ==> (lhs.numRules() != rhs.numRules() || EXISTS(0, lhs.numRules(), i, lhs.getRuleById(i) && rhs.ruleId(*lhs.getRuleById(i)) < 0)))
