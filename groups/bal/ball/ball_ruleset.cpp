...
// ACCESSORS
(__out != -1 ==> EXISTS(0, d_ruleAddresses.size(), i, d_ruleAddresses[i] == &*d_ruleHashtable.find(value))) && (__out == -1 ==> (d_ruleHashtable.find(value) == d_ruleHashtable.end() || FORALL(0, d_ruleAddresses.size(), i, d_ruleAddresses[i] != &*d_ruleHashtable.find(value))))
...
// FREE OPERATORS
(__out == true ==> (lhs.numRules() == rhs.numRules() && SFORALL(0, lhs.numRules(), i, EXISTS(0, rhs.numRules(), j, rhs.ruleId(*lhs.getRuleById(i)) == j)))) && (__out == false ==> (lhs.numRules() != rhs.numRules() || SEXISTS(0, lhs.numRules(), i, 0 > rhs.ruleId(*lhs.getRuleById(i)))))
