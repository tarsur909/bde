...
// FREE OPERATORS
(__out == true ==> (lhs.numContainers() == rhs.numContainers() && SEPFORALL(0, lhs.numContainers(), i, (*(std::next(lhs.begin(), i)) == *(std::next(rhs.begin(), i)))))) && (__out == false ==> (lhs.numContainers() != rhs.numContainers() || SEPEXISTS(0, lhs.numContainers(), i, (*(std::next(lhs.begin(), i)) != *(std::next(rhs.begin(), i))))))
