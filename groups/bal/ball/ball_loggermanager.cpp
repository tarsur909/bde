...
// requires: filteredNameBuffer != 0 && originalName != 0
// ensures: (nameFilter ? (__out == filteredNameBuffer->c_str() && filteredNameBuffer->back() == 0) : __out == originalName)
...
// ACCESSORS
// requires: true
// ensures: (__out == (category->relevantRuleMask() && (ThresholdAggregate::maxLevel(AttributeContext::getContext()->determineThresholdLevels(&ThresholdAggregate(), category)) >= severity) || (category->maxLevel() >= severity)))
