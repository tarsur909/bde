...
// requires: true
// ensures: (category.relevantRuleMask() == true ==> (__out == (ball::ThresholdAggregate::maxLevel(*levels) >= severity))) && (category.relevantRuleMask() == false ==> ((*levels == category.thresholdLevels()) && (__out == (category.maxLevel() >= severity))))
...
// ACCESSORS
// requires: true
// ensures: (category->relevantRuleMask() && __out == true) ==> category->maxLevel() >= severity || __out == false
