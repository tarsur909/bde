...
// ACCESSORS
(__out == true ==> (category->relevantRuleMask() ==> context->determineThresholdLevels(&levels, category) && ThresholdAggregate::maxLevel(levels) >= severity) || (!category->relevantRuleMask() && category->maxLevel() >= severity)) && (__out == false ==> (category->relevantRuleMask() ==> context->determineThresholdLevels(&levels, category) && ThresholdAggregate::maxLevel(levels) < severity) || (!category->relevantRuleMask() && category->maxLevel() < severity))
