...
// MANIPULATORS
// requires: SEPFORALL(0, v.size(), i, (v[i].key() ↦ _ ⋆ v[i].value() ↦ _)) && (SEPEXISTS(0, v.size(), i, v[i].value().isString()))
// ensures: (__out == -1 ==> SEPEXISTS(0, v.size(), i, !v[i].value().isString())) && (__out == 0 ==> (SEPFORALL(0, v.size(), i, v[i].value().isString())))
...
// CLASS METHODS
// requires: v == k_KEY_TIMESTAMP || v == k_KEY_PROCESS_ID || v == k_KEY_THREAD_ID || v == k_KEY_SEVERITY || v == k_KEY_FILE || v == k_KEY_LINE || v == k_KEY_CATEGORY || v == k_KEY_MESSAGE || v == k_KEY_ATTRIBUTES
...
// MANIPULATORS
// requires: format.empty() ==> res_tmp == -1
// ensures: format.empty() ==> __out == -1
