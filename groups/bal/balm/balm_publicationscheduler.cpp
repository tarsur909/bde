...
/// alphabetic order.
__out == &stream && FORALL(0, categoryList.size() - 1, k, u::CategorySort()(*categoryList[k], *categoryList[k + 1])) && FORALL(0, categories.size(), i, EXISTS(0, categoryList.size(), j, categoryList[j] == *std::next(categories.begin(), i)))
...
// ACCESSORS
(__out == false ==> d_categories.find(category) == d_categories.end()) && (__out == true ==> d_categories.find(category) != d_categories.end() && (*result == d_categories.find(category)->second))
bool
