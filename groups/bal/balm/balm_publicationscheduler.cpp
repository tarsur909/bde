...
/// alphabetic order.
__out == stream && SFORALL(0, categories.size(), i, EXISTS(categories.begin(), categories.end(), cat, __out.str().find(" " + (*cat)->name() + " ") != std::string::npos))
...
// ACCESSORS
(__out == false ==> d_categories.find(category) == d_categories.end()) && (__out == true ==> *result == d_categories.find(category)->second)
bool
