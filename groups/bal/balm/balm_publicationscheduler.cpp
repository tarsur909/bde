...
/// alphabetic order.
// requires: categories.size() >= 0 && SEPFORALL(0, categories.size(), i, (*std::next(categories.begin(), i))->name() ↦ _)
// ensures: __out == stream && SEPFORALL(0, categories.size(), i, EXISTS(0, categories.size(), j, (categoryList[j] == *std::next(categories.begin(), i)) && (stream + sep_v ↦ (*categoryList[j])->name())))
