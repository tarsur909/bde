...
// requires: buffer != 0 && length >= 0 && SEPFORALL(0, length, i, (buffer + i) ↦ _)
// ensures: (__out == true ==> SEPFORALL(0, length, i, bdlb::CharType::isPrint(buffer[i]))) && (__out == false ==> SEPEXISTS(0, length, i, !bdlb::CharType::isPrint(buffer[i])))
static
...
// CLASS METHODS
// requires: zoneinfoResult != nullptr && stream.good()
