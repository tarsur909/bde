...
// ensures: (__out == 0 ==> *rc != 0) && (__out != 0 ==> (__out->identifier() == timeZoneId ⋆ __out ↦ _))
...
// ensures: (__out == 0 ==> d_cache.find(timeZoneId) == d_cache.end()) && (__out != 0 ==> d_cache.find(timeZoneId) != d_cache.end() && __out == d_cache.find(timeZoneId)->second)
