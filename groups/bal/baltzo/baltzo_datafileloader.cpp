...
// requires: result != 0 && strlen(timeZoneId) > 0
// ensures: (__out == u::UNSUPPORTED_ID || __out == u::UNSPECIFIED_ERROR ==> result->identifier() == old_result->identifier()) && (__out != u::UNSUPPORTED_ID && __out != u::UNSPECIFIED_ERROR ==> result->identifier() == timeZoneId)
...
// CLASS METHODS
// requires: path != 0
// ensures: (__out == true ==> bdls::FilesystemUtil::isDirectory(path, true) && bdls::FilesystemUtil::isRegularFile(std::string(path) + "/GMT", true)) && (__out == false ==> !(bdls::FilesystemUtil::isDirectory(path, true) && bdls::FilesystemUtil::isRegularFile(std::string(path) + "/GMT", true)))
