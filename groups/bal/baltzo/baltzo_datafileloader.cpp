...
// CLASS METHODS
// requires: path != 0
// ensures: (__out == true ==> bdls::FilesystemUtil::isDirectory(path, true) && bdls::FilesystemUtil::isRegularFile((std::string(path) + "/GMT").c_str(), true)) && (__out == false ==> !(bdls::FilesystemUtil::isDirectory(path, true) && bdls::FilesystemUtil::isRegularFile((std::string(path) + "/GMT").c_str(), true)))
