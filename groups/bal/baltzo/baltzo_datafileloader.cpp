...
(__out == u::UNSUPPORTED_ID || __out == u::UNSPECIFIED_ERROR || __out == baltzo::ZoneinfoBinaryReader::readRaw(result, bsl::ifstream()) || __out == baltzo::ZoneinfoBinaryReader::read(result, bsl::ifstream()))
...
// CLASS METHODS
(__out == true ==> bdls::FilesystemUtil::isDirectory(path, true) && bdls::FilesystemUtil::isRegularFile((path + std::string("/GMT")).c_str(), true)) && (__out == false ==> !bdls::FilesystemUtil::isDirectory(path, true) || !bdls::FilesystemUtil::isRegularFile((path + std::string("/GMT")).c_str(), true))
