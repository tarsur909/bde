...
// requires: (lhs > rhs ==> lhs > rhs) && (lhs <= rhs ==> lhs <= rhs)
// ensures: __out == lhs || __out == rhs && (lhs > rhs ==> __out == lhs) && (lhs <= rhs ==> __out == rhs)
inline
...
// requires: true
// ensures: (endDate > beginDate ==> __out >= 0) && (endDate < beginDate ==> __out <= 0)
