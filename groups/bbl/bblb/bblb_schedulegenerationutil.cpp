...
(numerator >= 0 || numerator % denominator == 0 || __out == numerator/denominator - 1) && __out == (numerator / denominator) - (numerator < 0 && numerator % denominator != 0)
inline
...
/// `denominator`, without the use of floating-point calculations.  The
/// behavior is undefined unless `denominator > 0`.
__out == (numerator > 0 && numerator % denominator != 0 ? numerator/denominator + 1 : numerator/denominator)
inline
...
(__out == 0 ==> (*startSerialMonth <= u::k_MAX_SERIAL_MONTH && *endSerialMonth >= u::k_MIN_SERIAL_MONTH)) && (__out == 1 ==> (*startSerialMonth > u::k_MAX_SERIAL_MONTH || *endSerialMonth < u::k_MIN_SERIAL_MONTH))
